...
async function rollback(config) {
  try {
    ...
    console.log(`=====回退开始===${new Date().toLocaleString()}===`)
    const existsBackup = await sftp.exists(config.backupFolder)
    if (!existsBackup) {
      console.log(`=====备份文件夹不存在===${new Date().toLocaleString()}===`)
      return
    }
    ...
    if (latestBackupFolder) {
      ...
      if (existsDist) {
        ...
        if (!result) {
          console.log(`!!!删除 dist 文件夹失败!!!===${new Date().toLocaleString()}===`)
          return
        }
      }
      const result = await sftp.rename(`${config.backupFolder}/${latestBackupFolder.name}`, config.remotePath)
      if (result) {
        console.log(`=====回退成功===${new Date().toLocaleString()}===`)
      } else {
        console.log(`!!!回退失败!!!===${new Date().toLocaleString()}===`)
      }
    } else {
      console.log(`!!!未找到备份文件夹!!!===${new Date().toLocaleString()}===`)
    }
  } catch (error) {
